/*
* File Name   : StandRepComToSQL.cs
* 
* Description : share report query
* 
* Author      : yyan 
* 
...
/// <summary>
/// sql statement for query share report result.
/// </summary>
public class StandRepComToSQL
{
    ...
    /// <summary>
    /// get sql statement for query share report result related to BookingsSales view.
    /// </summary>
    ...
    /// <param name="strUserID">user id who share report</param>
    /// <returns></returns>
    public string RepCompileSql(string RepID, string RSMID, string CountryID, string CustomerID, string orderby,
                                string SelectMeetingDate, string strUserID)
    {
        try
        {
            string selrep = string.Format("select * from  ReportList WHERE ID={0}", RepID);
            string selrepv =
                string.Format("select * from  StandardReportValue WHERE ID={0} and userID ={1} ORDER BY FieldOrder",
                              RepID, strUserID);
  ...
  SalesOrg_User.UserID={2})",
                    ...
                    if (!string.IsNullOrEmpty(orderby))
                    {
                        ...
                        if (orderSplit.Length > 2)
                        {
                            string selOrder =
                                string.Format(
                                    "select FieldName from StandardReportValue WHERE ID={0} and userID ={1} and NewFieldName={2}",
                                    RepID, strUserID, "'" + orderSplit[1] + "'");
    ...
    /// <summary>
    /// get filter condition by report values
    /// </summary>
    /// <param name="tab">report values</param>
    /// <returns>filter condition</returns>
    public string getWhereNames(DataTable tab)
    ...
    /// <summary>
    /// get select meeting data by user id.
    /// </summary>
    /// <param name="userID">user id</param>
    /// <returns>select meeting data</returns>
    ...
    /// <summary>
    /// get sql query script for for query share report result except BookingsSales view and SalesBacklog view.
    /// </summary>
    /// <param name="RepID">report id</param>
    /// <param name="RSMID">user id</param>
    /// <param name="CountryID">Country ID</param>
        ...
        try
        {
            ...
            if (tabrep != null && selrepv != null)
            {
                if (tabrep.Rows.Count > 0 && tabrepv.Rows.Count > 0)
                {
                    ...
                    string ordername = getSortNames(tabrepv);
                    string sqlselect = string.Format("select {0} from {1}", fieldname, viewname);
                    string sqlwhere = string.Format(" where 0=0 {0} ", wherename);
                    string sqlorder = "";
                    if (!string.IsNullOrEmpty(orderby))
                    {
                        string[] orderSplit = orderby.Split(' ');
    ...
    /// <summary>
    /// get sql query script for query standard report result related to SalesBacklog view.
    /// </summary>
    /// <param name="RepID">report id</param>
    /// <param name="RSMID">user id</param>
    /// <param name="orderby">order condition</param>
